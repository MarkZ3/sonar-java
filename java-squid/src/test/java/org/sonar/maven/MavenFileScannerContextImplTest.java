...
public class MavenFileScannerContextImplTest {
  ...
  public void setup() {
    ...
    context = new MavenFileScannerContextImpl(mock(MavenProject.class), mock(SourceFile.class), mock(File.class), sonarComponents);
  }
  ...
  @Test
  public void should_do_nothing_when_there_is_no_sonarComponents() throws Exception {
    context = new MavenFileScannerContextImpl(mock(MavenProject.class), mock(SourceFile.class), mock(File.class), null);
    context.reportIssueOnFile(CHECK, "message");
    context.reportIssue(CHECK, 10, "message");
    context.reportIssue(CHECK, new LocatedTreeImpl() {
    }, "message");
    context.reportIssue(CHECK, 10, "message", new ArrayList<Location>());
  }
