...
public abstract class AVLTree<K, V> implements PMap<K, V>, PSet<K> {
  ...
  private class NodeIterator implements Iterator<Map.Entry<K, V>> {
    ...
    public Map.Entry<K, V> next() {
      ...
      AVLTree node = stack.removeFirst();
      if (node.right() != EMPTY) {
  ...
  private static class Node extends AVLTree {
    ...
    private class Hasher implements PMap.Consumer<Object, Object> {
      int result = 0;
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof Node) {
        Node other = (Node) obj;
        int c = 0;
        for (Iterator<Map.Entry> iter = iterator(); iter.hasNext();) {
        ...
        return c == Iterators.size(other.iterator());
      }
      return false;
    }
